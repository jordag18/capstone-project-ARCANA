...
class DatabaseManager:
    def __init__(self, db_name, host=HOST, port=PORT):
    ...
    def modify_event_from_project(self, project_name, event_id, updated_data):
        ...
        try:
            ...
            array_filters = [
                {
                    "elem._id": event_id_obj
                }  # Identifies the correct event in the event_list array by _id
            ]
            # Perform the update operation
            result = self.projects_collection.update_one(
                {
                    "name": project_name
                },  # Filter to identify the correct project document
                update_operation,
            ...
            event_after_update = (
                project_after_update["event_list"][0]
                if "event_list" in project_after_update
                and len(project_after_update["event_list"]) > 0
                else None
            )
            project = ProjectRepresenter.objects(name=project_name).first()
    ...
    def delete_icon(self, project_id: str, category: str, name: str):
        ...
        if project:
            try:
                ...
                if category in icon_library and name in icon_library[category]:
                    ...
                    for event in events:
                        ...
                        if event.action_title == name and event.team == category:
                            ...
                            event.action_title = (
                                f"{category.capitalize()} Team Activity"
                            )
                            event.icon = (
                                default_icon
                                or f"{category.lower().replace(' ', '-')}-team-activity.png"
                            )
                    return True
                else:
                    print(
                ...
                print(
                    "An error occurred while deleting the icon from the icon library:",
                    e,
                )
                return False
        else:
