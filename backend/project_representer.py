...
>>>>>>> 0a3e198 (Added graph algorithm)
    ...
    def update_graph(
        self, auto_edges, delete_node=None, edited_id=None, edited_data=None
    ):
...
>>>>>>> 0a3e198 (Added graph algorithm)
    ...
    def record_to_logger(
        ...
        try:
            match (change):
                case "ingested_logs":
                    statement = (
                        f"Ingested log file {data_source} in Project {self.name}"
                    )
                case "added_event":
            ...
            userActivityLogger.add_user_activity_log(
                initials="SYS", timestamp=datetime.now(), statement=statement
            )
    ...
    def add_toa_icon(self, team, action_title, icon_filename, is_default=False):
        try:
            ...
            if action_title.lower() in toa_icon_library[team.lower()]:
                raise ValueError(
                    f"Action title '{action_title}' already exists for the {team.lower()} team."
                )

            # Add the icon to the TOA icon library
            toa_icon_library[team.lower()][action_title.lower()] = {
                "image": icon_filename,
    ...
    def edit_toa_icon(
        ...
        try:
            ...
            if (
                old_team in self.toa_icon_library
                and old_action_title in self.toa_icon_library[old_team]
            ):
                ...
                old_icon_info["image"] = (
                    new_icon_filename if new_icon_filename else old_icon_info["image"]
                )
                old_icon_info["isDefault"] = (
                    new_is_default
                    if new_is_default is not None
                    else old_icon_info["isDefault"]
                )

                # Add the updated icon info to the new team under the new action title
                self.toa_icon_library[new_team if new_team else old_team][
                    new_action_title if new_action_title else old_action_title
                ] = old_icon_info
                ...
                if new_action_title or new_icon_filename:
                    for event in self.event_list:
                        if (
                            event.team == old_team
                            and event.action_title == old_action_title
                        ):
                            event.action_title = (
                                new_action_title
                                if new_action_title
                                else old_action_title
                            )
                            event.icon_filename = (
                                new_icon_filename
                                if new_icon_filename
                                else old_icon_info["image"]
                            )
                self.save()
            else:
                print(
                    f"Team '{old_team}' with action title '{old_action_title}' does not exist in the library."
                )
        except Exception as e:
            return {"error_message": f"Error occurred: {e}"}
