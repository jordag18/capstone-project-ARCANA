...
import {
  CreateToa,
  EditToa,
} from "@/app/components/toaComponents/toa-interface";
...
const IconLibrary = () => {
  ...
  const countDefaults = (team: string) => {
    ...
    const defaultCount = Object.values(icons).filter(
      (icon) => icon.isDefault
    ).length;
    return defaultCount;
  };
  ...
  const handleDeleteIcon = async (team: string, iconName: string) => {
    try {
      ...
      if (countsBefore < 2) {
        alert(
          "Default count should be greater than 1. Please edit another icon to be the new default."
        );
        return;
      }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center" }}>
        <h1 style={{ fontWeight: "bold", marginLeft: "2rem" }}>
          TOA Icon Library
        </h1>
        <button
          onClick={() => handleCreateModal(newToa || {})}
          style={{ marginLeft: "2rem" }}
          className="btn bg-gray-300 shadow-md hover:bg-gray-200 ml-2">
          + Create TOA
        </button>
        <button
          onClick={fetchIconLibrary}
          style={{ marginLeft: "1rem" }}
          className="btn bg-blue-300 shadow-md hover:bg-gray-200 ml-2">
          Reload
        </button>
        {isCreateModalOpen && (
          <CreateTOAModal
            ...
            <div
              ...
              {Object.entries(icons).map(([iconName, iconInfo]) => (
                <div
                  ...
                  <p>{iconName}</p>
                  {iconInfo.isDefault && (
                    <p style={{ color: "gray", fontSize: "12px" }}> default</p>
                  )}
                  <button
                    onClick={() => handleEditModal(team, iconName)}
                    className="btn bg-gray-300 shadow-md hover:bg-gray-200 ml-2">
                    Edit
                  </button>
                  {isEditModalOpen && (
                  ...
                  <button
                    onClick={() => handleDeleteIcon(team, iconName)}
                    style={{ marginLeft: "1rem" }}
