...
const IngestLogModal = ({
    ...
    const handleFileInputChange = (logFiles: React.ChangeEvent<HTMLInputElement>) => {
        const newFiles = logFiles.target.files;

        if (newFiles && newFiles.length > 0) {
            const newFilesArray = Array.from(newFiles).filter(file => file.name.endsWith('.csv'));
    ...
    return (
        <>
            <button className="btn" onClick={() => document.getElementById('my_modal_2').showModal()}>Ingest Logs</button>
            <dialog id="my_modal_2" className="modal">
                <div className="modal-box">
                            ...
                            <div>
                                ...
                                <ul>
                                    {files.map((file, index) => (
                                        <li key={index}>
                                            {file.name}
                                            <button className="btn-sm" onClick={() => handleRemoveFile(index)}>
                    ...
                    <div className="mb-3 d-flex space-x-2">
                        <button className="btn" onClick={() => { handleIngestLogs(); document.getElementById('my_modal_2').close() }}>
                            Ingest Logs
                        </button>
                        <button className="btn bg-red-600 text-white" onClick={handleClearLogs}>
                            Clear Logs
                        </button>
                    </div>
                </div>
